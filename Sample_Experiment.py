...
from transformers import (
    ...
    MBartTokenizer, MBart50Tokenizer,
    MBartTokenizerFast, MBartForConditionalGeneration,
    SchedulerType,
...
def main():
    # Parse the arguments
    args = parse_args()
    args.model_name_or_path = "C:/PythonProject/mbart-large-50"
    ...
    if args.tokenizer_name:
        tokenizer = MBart50Tokenizer.from_pretrained(args.tokenizer_name, use_fast=not args.use_slow_tokenizer)
    elif args.model_name_or_path:
        tokenizer = MBart50Tokenizer.from_pretrained(args.model_name_or_path, use_fast=not args.use_slow_tokenizer)
    else:
        raise ValueError(
    ...
    processed_datasets = raw_datasets['test'].map(
        preprocess_function,
        batched=True,
    ...
    pbar = ProgressBar(n_total=args.num_train_epochs * len(train_dataloader))
    for epoch in range(args.num_train_epochs):
        model.train()
        for step, batch in enumerate(train_dataloader):
            outputs = model(input_ids=batch['input_ids'].cuda(), attention_mask=batch['attention_mask'].cuda(),
                            labels=batch['labels'].cuda(), forced_bos_token_id=tokenizer.lang_code_to_id["ro_RO"])
            loss = outputs.loss
            loss = loss / args.gradient_accumulation_steps
            loss.backward()
            pbar(epoch * len(train_dataloader) + step, {'loss': loss.item()})
